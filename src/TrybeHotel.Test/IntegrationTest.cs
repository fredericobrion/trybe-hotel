...
public class IntegrationTest : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Cria uma cidade")]
    [InlineData("/city")]
    ...
    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Lista os hotéis")]
    [InlineData("/hotel")]
    ...
    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Cria um hotel")]
    [InlineData("/hotel")]
    public async Task TestPostHotel(string url)
    {
        var response = await _clientTest.PostAsync(url, new StringContent(JsonConvert.SerializeObject(new Hotel { HotelId = 4, Name = "Trybe Hotel São Paulo", Address = "Address 4", CityId = 2 }), Encoding.UTF8, "application/json"));
        Assert.Equal(System.Net.HttpStatusCode.Created, response?.StatusCode);
    }


    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Não cria um hotel quando o id da cidade não existir no DB")]
    [InlineData("/hotel")]
    public async Task TestPostHotelWithoutCity(string url)
    {
        var response = await _clientTest.PostAsync(url, new StringContent(JsonConvert.SerializeObject(new Hotel { HotelId = 4, Name = "Trybe Hotel São Paulo", Address = "Address 4", CityId = 3 }), Encoding.UTF8, "application/json"));
        Assert.Equal(System.Net.HttpStatusCode.NotFound, response?.StatusCode);
    }


    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Lista os quartos")]
    [InlineData("/room/1")]
    ...
    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Cria um quarto")]
    [InlineData("/room")]
    ...
    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Não cria um quarto quando o id do hotel não existir no DB")]
    [InlineData("/room")]
    public async Task TestPostRoomWithoutHotel(string url)
    {
        var response = await _clientTest.PostAsync(url, new StringContent(JsonConvert.SerializeObject(new Room { RoomId = 10, Name = "Room 10", Capacity = 2, Image = "Image 10", HotelId = 4 }), Encoding.UTF8, "application/json"));
        Assert.Equal(System.Net.HttpStatusCode.NotFound, response?.StatusCode);
    }


    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Deleta um quarto")]
    [InlineData("/room/1")]
